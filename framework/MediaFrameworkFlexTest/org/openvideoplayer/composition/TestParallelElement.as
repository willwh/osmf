...
package org.openvideoplayer.composition
{
	...
	public class TestParallelElement extends TestCompositeElement
	{
		...
		public function testGetTraitLoadable():void
		{
			...
			var mediaElement1:MediaElement = 
				new DynamicMediaElement([MediaTraitType.LOADABLE],
										loader1,
										new URLResource("http://www.example.com/loadable1"));
			...
			var mediaElement4:MediaElement =
				new DynamicMediaElement([MediaTraitType.LOADABLE],
				new SimpleLoader(),
				new URLResource("http://www.example.com/loadable4"));
		...
		public function testGetTraitPlayable():void
		{
			...
			// playing state if it has the IPausible trait.)  So given the
		...
		public function testGetTraitSpatial():void
		{
			...
			assertEquals(600, spatialP.width);
			assertEquals(600, spatialP.height);
			
			spatial2.setDimensions(1000,1000);
			...
			assertEquals(spatialP.width, 1000);
			assertEquals(spatialP.height, 1000);
		...
		public function testGetTraitViewable():void
		{
			...
			var viewableTrait:IViewable = parallel.getTrait(MediaTraitType.VIEWABLE) as IViewable; 
		...
		public function testGetTraitPausible():void
		{
			...
			// No trait to begin with.
			assertTrue(parallel.getTrait(MediaTraitType.PAUSIBLE) == null);
			...
			// propagate to the composition.
			parallel.addChild(mediaElement1);
			var pausible:IPausible = parallel.getTrait(MediaTraitType.PAUSIBLE) as IPausible;
			var playable:IPlayable = parallel.getTrait(MediaTraitType.PLAYABLE) as IPlayable;
			...
			assertTrue(pausible2.paused == true);
			assertTrue(pausible.paused == true);
