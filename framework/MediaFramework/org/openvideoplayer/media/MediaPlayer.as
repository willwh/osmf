...
package org.openvideoplayer.media
{
	 ...
	 */	
	[Event(name="playerStateChange", type="org.openvideoplayer.events.MediaPlayerStateChangeEvent")]
	...
	/**
	 * Dispatched when the <code>pausible</code> property has changed.
	 * 
	 * @eventType org.openvideoplayer.events.MediaPlayerCapabilityChangeEvent.PAUSIBLE_CHANGE
	 */
	[Event(name="pausibleChange", type="org.openvideoplayer.events.MediaPlayerCapabilityChangeEvent")]
	 ...
	 *  
	 * <p>The MediaPlayer can play back all media types supported by the Strobe media framework, 
	...
	public class MediaPlayer extends EventDispatcher
	{
		...
		/**
		 *  Indicates whether the media is pausible.
		 */		
	    ...
	    {	
	    	return (getTrait(MediaTraitType.AUDIBLE) as IAudible).volume;	    		    
	    }		   
	    ...
	    {
	    	var trait:IAudible = getTrait(MediaTraitType.AUDIBLE) as IAudible;
	    	return trait.muted;		   
	    }
		 ...
		 * pausible to support this property.</p>
		 * 
		 * @throws IllegalOperationError if capability isn't supported
         * @see org.openvideoplayer.traits.IPausible
		 */	
		public function get paused():Boolean
	    {
	    	return (getTrait(MediaTraitType.PAUSIBLE) as IPausible).paused;	    		    
	    }
	    ...
	    */
	    ...
	    {
	    	(getTrait(MediaTraitType.PAUSIBLE) as IPausible).pause();	    		
	    }
	    ...
	    */	    
		...
		{		
			...
			switch (trait)
			{
				...
				case MediaTraitType.PAUSIBLE:											
					changeListeners(add, _source, MediaTraitType.PAUSIBLE, PausedChangeEvent.PAUSED_CHANGE, [redispatchEvent, onPaused]);						
					_pausible = add;		
					traitChangeName = MediaPlayerCapabilityChangeEvent.PAUSIBLE_CHANGE;	
					break;
				...
				case MediaTraitType.LOADABLE:					
					changeListeners(add, _source, MediaTraitType.LOADABLE, LoadableStateChangeEvent.LOADABLE_STATE_CHANGE, [onLoadState]);					
					if (add && (_source.getTrait(MediaTraitType.LOADABLE) as ILoadable).loadState != LoadState.LOADED )
					{
				...
				case MediaTraitType.BUFFERABLE:
					changeListeners(add, _source, MediaTraitType.BUFFERABLE, BufferingChangeEvent.BUFFERING_CHANGE, [redispatchEvent]);	
		...
		{			
			...
			else if (autoRewind && seekable && pausible)
			{				
				seek(0);
				pause();				
			}	
