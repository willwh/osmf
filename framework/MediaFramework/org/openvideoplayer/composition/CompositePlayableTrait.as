			...
			else if (playable == traitOfCurrentChild)
			{
				...
				else
				{
					...
					else
					{
						...
						var oldCurChild:MediaElement = traitAggregator.listenedChild;
						var oldTemporal:ITemporal = oldCurChild.getTrait(MediaTraitType.TEMPORAL) as ITemporal;
						var oldPausible:IPausible = oldCurChild.getTrait(MediaTraitType.PAUSIBLE) as IPausible;
						if (oldPausible != null && 
							oldTemporal != null &&
							(oldPausible.paused && (oldTemporal.position < oldTemporal.duration)))
						{
							return;		
						}
