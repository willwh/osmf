...
package org.openvideoplayer.composition
{
	...
	public class CompositeElement extends MediaElement
	{
		...
		{
			var mediaTrait:IMediaTrait = getTrait(traitType);
			if (mediaTrait != null)
			{
				if (traitType == MediaTraitType.PLAYABLE)
				{
					mediaTrait.removeEventListener(PlayingChangeEvent.PLAYING_CHANGE, onPlayingChanged);
				}
				else if (traitType == MediaTraitType.PAUSIBLE)
				{
					mediaTrait.removeEventListener(PausedChangeEvent.PAUSED_CHANGE, onPausedChanged);
				}
			}
		}
		
		protected function postProcessAggregatedTrait(traitType:MediaTraitType, trait:IMediaTrait):void
		{
			if (traitType == MediaTraitType.PLAYABLE)
			{
				trait.addEventListener(PlayingChangeEvent.PLAYING_CHANGE, onPlayingChanged);
			}
			else if (traitType == MediaTraitType.PAUSIBLE)
			{
				trait.addEventListener(PausedChangeEvent.PAUSED_CHANGE, onPausedChanged);
			}
		}
		...
		{
			var pausible:CompositePausibleTrait = getTrait(MediaTraitType.PAUSIBLE) as CompositePausibleTrait;
		...
		{
			var playable:CompositePlayableTrait = getTrait(MediaTraitType.PLAYABLE) as CompositePlayableTrait;
			if (event.paused && playable != null)
			{
