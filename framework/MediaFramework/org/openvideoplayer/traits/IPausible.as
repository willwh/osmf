...
package org.openvideoplayer.traits
{
	...
	/**
	 * IPausible defines the trait interface for media that can be paused. 
	 * 
	 * <p>Use the <code>MediaElement.hasTrait(MediaTraitType.PAUSIBLE)</code> method to query
	 * whether a media element has a trait that implements this interface. 
	 * If <code>hasTrait(MediaTraitType.PAUSIBLE)</code> returns <code>true</code>,
	 * use the <code>MediaElement.getTrait(MediaTraitType.PAUSIBLE)</code> method
	 * to get an object that is guaranteed to implement the IPausible interface.</p>
	 * <p>Through its MediaElement, an IPausible trait can participate in media compositions.
	 ...
	 */	
	public interface IPausible extends IMediaTrait
	{
		/**
