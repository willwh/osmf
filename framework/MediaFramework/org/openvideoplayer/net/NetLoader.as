...
package org.openvideoplayer.net
{
	...
	import org.openvideoplayer.events.NetConnectionFactoryEvent;
	...
	public class NetLoader extends LoaderBase
	{
		 ...
		 * method. If the protocol is RTMP-based, calls the  <code>startLoadingRTMP()</code> method. Ifthe URL protocol is invalid,
	     ...
	     * @see org.openvideoplayer.events.MediaErrorEvent
		 * @inheritDoc
		**/
		...
		**/
		...
		/**
		 * The NetLoader returns true for IURLResources which supportr the media and mime-typesimplement one
		...
		**/
		...
		{
			var rtAudio:int 
				= MetadataUtils.checkMetadataMatchWithResource(resource, AUDIO_MEDIA_SUPPORTED, AUDIO_MIME_TYPES_SUPPORTED);
			var rtVideo:int 
				= MetadataUtils.checkMetadataMatchWithResource(resource, VIDEO_MEDIA_SUPPORTED, VIDEO_MIME_TYPES_SUPPORTED);
			if (rtAudio != MetadataUtils.METADATA_MATCH_UNKNOWN && rtVideo != MetadataUtils.METADATA_MATCH_UNKNOWN)
			{
				return (rtAudio == MetadataUtils.METADATA_MATCH_FOUND || rtVideo == MetadataUtils.METADATA_MATCH_FOUND);
			}			
		...
		**/
		...
		private static const AUDIO_MIME_TYPES_SUPPORTED:Vector.<String> = Vector.<String>([
			"audio/mpeg"
		]);
		...
		private static const AUDIO_MEDIA_SUPPORTED:Vector.<String> = Vector.<String>([MediaType.AUDIO]);
		private static const VIDEO_MEDIA_SUPPORTED:Vector.<String> = Vector.<String>([MediaType.VIDEO]);
