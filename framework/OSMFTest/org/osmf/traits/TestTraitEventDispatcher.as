...
package org.osmf.traits
{
	...
	public class TestTraitEventDispatcher extends TestCase
	{
		...
		{			
			...
			assertEquals(1, caughtEvents.length);
			assertEquals(Event.ENTER_FRAME, caughtEvents[0].type);
		...
		private function testWithElement(events:Vector.<Event>, properties:Vector.<String>, traitType:String):void
		{
			...
			assertEquals(events.length, caughtEvents.length);
						
			for (itr = 0; itr < events.length; itr++)
			{
				assertEquals(caughtEvents[itr].type,  events[itr].type );
				assertTrue(caughtEvents[itr] is (getDefinitionByName(getQualifiedClassName(events[itr])) as Class) );
				for (itr2 = 0; itr2 < properties.length; itr2++)
				{
					assertEquals(events[itr][properties[itr2]], caughtEvents[itr][properties[itr2]]);
				}
			}			
