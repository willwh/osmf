...
package org.osmf.net.httpstreaming
{
	 ...
	 */ 
	public class HTTPStreamProvider 
	{
		/**
		 * Default constructor.
		 */
		public function HTTPStreamProvider(factory:HTTPStreamingFactory, resource:MediaResourceBase, dispatcher:IEventDispatcher)
		{
			if (dispatcher == null)
			{
			...
			setState(HTTPStreamProviderState.INIT);
		...
		public function get qosInfo():HTTPStreamProviderQoSInfo
		{
			return _qosInfo;
		}
		...
		public function seek(offset:Number):void
		{
			...
			setState(HTTPStreamProviderState.SEEK);
		}
		...
		public function getBytes():ByteArray
		{
			...
			if (_state != HTTPStreamProviderState.INIT)
			{
		...
		protected function doSomeProcessingAndGetBytes():ByteArray
		{
			...
			switch(_state)
			{
				case HTTPStreamProviderState.SEEK:
					...
					{
						_source.close();
						_fileHandler.flushFileSegment(_source.getBytes());
					}
					
					setState(HTTPStreamProviderState.LOAD);
					break;

				case HTTPStreamProviderState.WAIT:
					date = new Date();
				...
				case HTTPStreamProviderState.LOAD:
					...
					if (_request != null && _request.urlRequest != null)
					{
						...
						if (_source == null)
						{
							_source = new HTTPStreamSource(_dispatcher);
						}
						_source.open(_request.urlRequest);
						setState(HTTPStreamProviderState.BEGIN_FRAGMENT);
					}
					else if (_request != null && _request.retryAfter >= 0)
					{
						...
						setState(HTTPStreamProviderState.WAIT);
					}
					else
					{
						...
						setState(HTTPStreamProviderState.STOP);	
					}
					break;
				
				case HTTPStreamProviderState.BEGIN_FRAGMENT:
					_endFragment = false;
					if (_seekTarget != -1)
					{
					...
					setState(HTTPStreamProviderState.READ);
					break;
				
				case HTTPStreamProviderState.READ:
					if (_source != null)
					{
						input =  _source.getBytes(_fileHandler.inputBytesNeeded);
						if (input != null)
						{
					...
					if (_state == HTTPStreamProviderState.READ)
					{
						if (_endFragment)
						{
							if (_source != null)
							...
							setState(HTTPStreamProviderState.END_FRAGMENT);
						}
					}
					break;
				
				case HTTPStreamProviderState.END_FRAGMENT:
					if (_source != null)
					{
						input = _source.getBytes();
						if (input != null)
						{
					...
					_qosInfo = new HTTPStreamProviderQoSInfo(_fragmentDuration, _source.downloadBytesCount, _source.downloadDuration);
		...
		private var _qosInfo:HTTPStreamProviderQoSInfo = null;
		...
		CONFIG::LOGGING
		{
			private static const logger:Logger = Log.getLogger("org.osmf.net.httpstreaming.HTTPStreamProvider");
