package
{
	...
	public class VPAIDSample extends Sprite
	{
		...
		{
			trace("Starting VPAIDSample Video Player");
			...
			vpaidLinearResource = new URLResource(chosenLinear);
			vpaidNonLinearResource = new URLResource(chosenNonLinear);
		...
		{
			trace("VPAIDSample.onLoadStateChange " + e.loadState);	
			if(e.loadState == LoadState.UNINITIALIZED)
			{
			
				vpaidPrerollLinear = null;
				trace("VPAIDSample.onLoadStateChange");	
			}
		...
		{
			trace("VPAIDSample.onMetadataValueAdded() " + e.key);
			if(e.key == "error")
			{
		...
		{
			trace("VPAIDSample.onMetadataValueChange() " + e.target.getValue(e.key));
		
		}		
		
		private function onContentLoadChange(e:LoadEvent):void
		{
			trace("VPAIDSample.onContentLoadChange() " + e.loadState);
		}	
		
		private function onPlayStateChange(e:PlayEvent):void
		{
			trace("VPAIDSample.onPlayStateChange " + e.playState);	
		}		
