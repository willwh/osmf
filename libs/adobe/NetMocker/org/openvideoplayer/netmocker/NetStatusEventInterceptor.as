...
package org.openvideoplayer.netmocker
{
	...
	internal class NetStatusEventInterceptor
	{
		...
		{
			// Once a CONENCT_CLOSED event is event, we want to prohibit the dispatching of any events which have been
		...
		private function doDispatchNetStatusEvents(objectInfos:Array):void
		{
			for each (var objectInfo:Object in objectInfos)
			{
				...
				netStatusEventDispatcher.dispatchEvent(new NetStatusEvent
					...
					, false
					, false
					, {"code":objectInfo["code"], "level":objectInfo["level"], "mockEvent":true}
					)
				);
			}
		}
